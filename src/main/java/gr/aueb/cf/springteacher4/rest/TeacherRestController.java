...
public class TeacherRestController {
    ...
    public ResponseEntity<List<TeacherReadOnlyDTO>> getTeachersByLastname(@RequestParam("lastname") String lastname) {
        ...
        try {
            ...
            for (Teacher teacher : teachers) {
                teacherReadOnlyDTOS.add(convertToReadOnlyDto(teacher));
            }
            return new ResponseEntity<>(teacherReadOnlyDTOS, HttpStatus.OK);
    ...
    public ResponseEntity<TeacherReadOnlyDTO> addTeacher(@RequestBody TeacherInsertDTO dto, BindingResult bindingResult) {
        ...
        try {
            ...
            URI location = ServletUriComponentsBuilder.fromCurrentRequest()
                    .path("/{id}").buildAndExpand(teacherReadOnlyDTO.getId()).toUri();
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "Teacher not found",
                    content = @Content) })
    @RequestMapping(path = "/teachers/{id}", method = RequestMethod.POST)
    ...
    private TeacherReadOnlyDTO convertToReadOnlyDto(Teacher teacher) {
        TeacherReadOnlyDTO teacherReadOnlyDTO = new TeacherReadOnlyDTO(teacher.getFirstname(), teacher.getLastname());
